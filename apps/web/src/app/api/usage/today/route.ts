...
export async function GET(request: NextRequest) {
  ...
  if (!raw) {
    return withCors(NextResponse.json({ error: "Missing token" }, { status: 401 }), request);
  }

  const tokenUser = await validateExtensionToken(raw);
  if (!tokenUser) {
    return withCors(NextResponse.json({ error: "Invalid token" }, { status: 401 }), request);
  }

  const quota = await getQuotaState(tokenUser.userId, tokenUser.plan);
  return withCors(NextResponse.json(quota), request);
}
